...
def inject_glass_styles():
    st.markdown("""
        <style>
        /* MAIN BACKGROUND - Deep Space Black with animated gradients */
        .stApp {
            background: #000000;
            background-image: 
        ...
        /* 7. CUSTOM GLASS CARDS (For About Us & Metrics) */
        .glass-card-content {
            background: rgba(255,255,255,0.03);
        ...
        .score-key-green { background-color: rgba(46, 204, 113, 0.3); border-color: #2ecc71; }
        .score-key-yellow { background-color: rgba(241, 196, 15, 0.3); border-color: #f1c40f; }
        .score-key-orange { background-color: rgba(230, 126, 34, 0.3); border-color: #e67e22; }
        .score-key-red { background-color: rgba(231, 76, 60, 0.3); border-color: #e74c3c; }
...
def show_home_screen():
    st.markdown("""
        ...
        .blob-2 {
            bottom: 10%; right: 20%; width: 600px; height: 600px;
        ...
        .blob-3 {
            ...
            background: #ffffff; opacity: 0.15; border-radius: 50%;
            filter: blur(60px);
        }
        ...
        .title-text {
            font-family: 'Segoe UI', sans-serif; font-size: 5rem; font-weight: 900;
        ...
        .subtitle-text {
            color: rgba(255,255,255,0.8); font-size: 1.3rem; margin-bottom: 50px;
        ...
        <div class="glass-container">
            ...
            <div class="glass-card">
                ...
                <div class="subtitle-text">Intelligent Urban Navigation</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
if st.session_state.show_home:
    ...
    with tab1:
        ...
        # --- MAP & SIDEBAR ---
        m = folium.Map(location=[28.6139, 77.2090], zoom_start=11, tiles='cartodbdark_matter')
            ...
            if os.path.exists(json_path):
                try:
                    ...
                    folium.GeoJson(
                        ...
                        style_function=lambda feature: {"fillColor": "#ff0044", "color": "#ff0044", "weight": 1, "fillOpacity": 0.4},
        ...
        # Markers
        if st.session_state.start_point:
            folium.Marker(st.session_state.start_point, popup="Start", icon=folium.Icon(color="green", icon="play")).add_to(m)
        if st.session_state.end_point:
            folium.Marker(st.session_state.end_point, popup="Destination", icon=folium.Icon(color="red", icon="flag")).add_to(m)

        # Route Filtering & Drawing
        ...
        if st.session_state.analyzed and st.session_state.routes:
            ...
            # 1. Safety Score
            m1.markdown(f"""
                <div class="glass-card-content" style="text-align:center; border-color:#00ff88;">
                    <h2 style="margin:0; color:#00ff88; text-shadow:0 0 10px rgba(0,255,136,0.5);">{best['safety_score']}/100</h2>
                    <p style="margin:0; color:white; opacity:0.8; font-size:0.9rem;">Safety Score</p>
                </div>
            """, unsafe_allow_html=True)
            # 2. Distance
            m2.markdown(f"""
                <div class="glass-card-content" style="text-align:center;">
                    <h2 style="margin:0; color:white;">{best['distance']/1000:.2f} <span style="font-size:1rem; opacity:0.7">km</span></h2>
                    <p style="margin:0; color:white; opacity:0.8; font-size:0.9rem;">Total Distance</p>
                </div>
            """, unsafe_allow_html=True)
            # 3. Duration
            m3.markdown(f"""
                <div class="glass-card-content" style="text-align:center;">
                    <h2 style="margin:0; color:white;">{best['duration']/60:.0f} <span style="font-size:1rem; opacity:0.7">min</span></h2>
                    <p style="margin:0; color:white; opacity:0.8; font-size:0.9rem;">Estimated Time</p>
                </div>
            """, unsafe_allow_html=True)
    ...
    # --- TAB 3: ABOUT (Enhanced with Glass Cards) ---
    with tab3:
        st.header("👥 About Team SafePath")
        
        st.markdown("""
        <div class="glass-card-content">
            <h3>The Vision: Safety-First Navigation</h3>
            <p><b>Problem:</b> Traditional route planning prioritizes efficiency over human safety. For pedestrians traveling at night, this oversight is critical.</p>
            <p><b>Solution:</b> SafePath is an AI-Powered routing system that optimizes paths based on <b>risk minimization</b>. We combine real-time infrastructure data, crime stats, and environmental factors to empower safer travel decisions.</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown("### Meet the Team")
        st.markdown("We are a team of dedicated developers participating in the Hackathon to build practical solutions leveraging geospatial data.")
        ...
        def team_card(name, role):
            return f"""
            <div class="glass-card-content" style="padding:15px; text-align:center;">
                <h4 style="margin:0; color:#4facfe;">{name}</h4>
                <p style="margin:0; opacity:0.7; font-size:0.9rem;">{role}</p>
            </div>
            """

        with col_row_1[0]: st.markdown(team_card("Tanay Shah", "Lead Developer"), unsafe_allow_html=True)
        with col_row_1[1]: st.markdown(team_card("S Sanjay", "Front-end Engineer"), unsafe_allow_html=True)
        with col_row_2[0]: st.markdown(team_card("Kishaann Karthik", "Data Analyst"), unsafe_allow_html=True)
        with col_row_2[1]: st.markdown(team_card("Ishaan Bhatia", "Back-end Engineer"), unsafe_allow_html=True)
        
        st.info("Key Takeaway for Judges: SafePath moves beyond calculating the *shortest* path—it calculates the **smartest** path.")
